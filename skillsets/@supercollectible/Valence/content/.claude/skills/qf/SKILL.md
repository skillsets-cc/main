...
- **description**: Use `TeamCreate` with name `qf`. Create one task per teammate using `TaskCreate`, listing its assigned files in the task description. Spawn teammates using `subagent_type`, `model`, and `mode` from the `agents` field in this skill's frontmatter.
