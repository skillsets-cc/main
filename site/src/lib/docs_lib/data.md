...
## Integration Points
- **Used by**:
  - `pages/index.astro` (list all skillsets via `getSkillsets`)
  - `pages/skillset/[namespace]/[name].astro` (fetch individual skillset via `getSkillsetById`)
  - Note: `getAllTags` and `getSearchIndexMeta` are currently unused exports
- **Consumes**:
